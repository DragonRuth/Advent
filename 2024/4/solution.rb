...
lambda {|row,col,mtrx| row < 3 && v < 0 ||  row >= mtrx.row_count - 3  && v > 0|| col < 3 && h < 0||  col >=  mtrx.column_count - 3 && h > 0  ? false : (mtrx[row + v*1, col + h*1].nil? ? "" : mtrx[row + v*1, col + h*1]) + (mtrx[row + v*2, col + h*2].nil? ? "" : mtrx[row + v*2, col + h*2])  + (mtrx[row + v*3, col + h*3].nil?  ? "" : mtrx[row + v*3, col + h*3]) == "MAS"}
}
